...
{
    CType type;
    type.t = (r & VT_VALMASK) == VT_LOCAL ? VT_PTR : VT_INT;
    type.ref = 0;
    vset(&type, r, v);
}
        ...
        if (c) {
            ...
            if (df) {
                if ((sbt & VT_BTYPE) == VT_LLONG) {
                    if (sbt & VT_UNSIGNED)
                        vtop->c.ld = (long double)vtop->c.ull;
                    else
                        vtop->c.ld = (long double)vtop->c.ll;
                } else if(!sf) {
                    if (sbt & VT_UNSIGNED)
                        vtop->c.ld = vtop->c.ui;
                    else
...
#if !defined(TCC_TARGET_X86_64) && !defined(TCC_TARGET_SVM32) && !defined(TCC_TARGET_SVM64)
#ifndef TCC_TARGET_X86_64
...
#if defined(TCC_TARGET_SVM32) || defined(TCC_TARGET_SVM64)
					/* movslq/movzlq */
					gen_load_reg(REG_VALUE(r) | TREG_CAST64, REG_VALUE(r), sbt);
#else
                    if (sbt != (VT_INT | VT_UNSIGNED)) {
...
#endif
#if defined(TCC_TARGET_SVM32) || defined(TCC_TARGET_SVM64)
        case TOK_nativestdcall:
            ad->a.func_call = FUNC_NATIVECALL;
            break;            
        case TOK_nativethiscall:
            ad->a.func_call = FUNC_NATIVETHISCALL;
            break;            
        case TOK_naked:
            ad->a.func_naked = 1;
            break;            
...
the_end:
    ...
    if ((t & VT_BTYPE) == VT_LONG)
#if defined(TCC_TARGET_SVM32) || defined(TCC_TARGET_SVM32)
