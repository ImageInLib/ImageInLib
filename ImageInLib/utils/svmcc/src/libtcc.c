...
#include "i386-asm.c"
#endif
#if defined TCC_TARGET_SVM32 || defined TCC_TARGET_SVM64
#include "svm-asm.c"
#endif
#endif
...
#if defined(TCC_TARGET_SVM32) || defined(TCC_TARGET_SVM64)
			else if (ref->a.func_call == FUNC_NATIVECALL || ref->a.func_call == FUNC_NATIVETHISCALL)
			{
...
#if defined(TCC_TARGET_SVM32) || defined(TCC_TARGET_SVM64)
	tcc_listing_open(s1, file->filename);
#endif
...
#if defined(TCC_TARGET_SVM32) || defined(TCC_TARGET_SVM64)
	if (s1->nb_errors == 0)
		tcc_listing_dump_section(data_section);
	tcc_listing_close();
#endif
...
#endif
#elif defined(TCC_TARGET_SVM32)
    tcc_define_symbol(s, "__SVM32__", NULL);
#elif defined(TCC_TARGET_SVM64)
    tcc_define_symbol(s, "__SVM64__", NULL);
    tcc_define_symbol(s, "_M_X64", NULL);
#endif
...
#if defined(TCC_TARGET_SVM32) || defined(TCC_TARGET_SVM64)
	if (s1->gen_listing_path)
		tcc_free(s1->gen_listing_path);
#endif
    ...
    while (option && *option) {
        ...
        if (link_option(option, "Bsymbolic", &p)) {
            ...
            if (!strcmp(p, "native")) {
                ...
                s->pe_subsystem = 13;
#elif defined(TCC_TARGET_SVM32) || defined(TCC_TARGET_SVM64)
            if (!strcmp(p, "svm")) {
                s->pe_subsystem = 2;
#elif defined(TCC_TARGET_ARM)
            if (!strcmp(p, "wince")) {
...
#if defined(TCC_TARGET_SVM32) || defined(TCC_TARGET_SVM64)
    { "GL", TCC_OPTION_listing, TCC_OPTION_HAS_ARG | TCC_OPTION_NOSEP },
#endif
    { NULL, 0, 0 },
};
...
PUB_FUNC int tcc_parse_args(TCCState *s, int argc, char **argv)
{
    ...
    while (optind < argc) {
        ...
        case TCC_OPTION_x:
            ...
            break;
#if defined(TCC_TARGET_SVM32) || defined(TCC_TARGET_SVM64)
        case TCC_OPTION_listing:
			s->gen_listing = 1;
			tcc_listing_set_path(s, optarg);
            break;
#endif
        default:
...
PUB_FUNC void tcc_listing_set_path(TCCState *s, const char *path)
{
    tcc_free(s->gen_listing_path);
    s->gen_listing_path = tcc_strdup(path);
}

typedef struct GenListingState {
	FILE *f;
	int lastind;
} GenListingState;
static GenListingState *gen_lst = NULL;

static char *listing_filename(TCCState *s, const char *filename)
{
    char buf[1024];
    char *ext;
	...
	{
		pstrcpy(buf, sizeof(buf), s->outfile);
	}
...
PUB_FUNC void tcc_listing_open(TCCState *s, const char* filename)
{
	char *lstfile;
	FILE *f;
	...
	lstfile = listing_filename(s, filename);
    f = fopen(lstfile, "w");
    if (!f)
        tcc_error("could not write '%s'", lstfile);
	tcc_free(lstfile);

	gen_lst = (GenListingState*)tcc_malloc(sizeof(*gen_lst));
	gen_lst->f = f;
	gen_lst->lastind = 0;

	tcc_listing_print("FILE %s\n", filename);
}

PUB_FUNC void tcc_listing_close()
{
	if (gen_lst)
	{
		if (gen_lst->f)
...
PUB_FUNC void tcc_listing_print(const char *fmt, ...)
{
    va_list ap;
    ...
    va_start(ap, fmt);
	tcc_listing_vprint(fmt, ap);
    va_end(ap);
}

PUB_FUNC void tcc_listing_vprint(const char *fmt, va_list ap)
{
	if (!gen_lst)
		return;
	vfprintf(gen_lst->f, fmt, ap);
}

static void listing_dump(Section *section, int offset, int len, int symoffset, int bytesperline, int ascii)
{
	char buf[256];

	sprintf(buf, "  %s+0x%05x", section->name, offset);
	fwrite(buf, strlen(buf), 1, gen_lst->f);
	strset(buf, ' ');

	while (len)
	{
		int m = min(len, bytesperline);
		int n;
        for (n = 0; n < bytesperline; ++n)
		{
			if (n >= m)
				fwrite("   ", 3, 1, gen_lst->f);
			else
				fprintf(gen_lst->f, "%c%02x",
					(symoffset > 0 && offset+n >= symoffset && offset+n < symoffset+4) ? '*' : ' ',
					(int)(unsigned char)section->data[offset+n]);
		}
		if (ascii)
		{
			fwrite("  '", 3, 1, gen_lst->f);
			for (n = 0; n < m; ++n)
			{
                unsigned char b = section->data[offset+n];
                if (b < 32 || b >= 127)
                    b = '.';
                fputc(b, gen_lst->f);
			}
		...
		{
			fwrite("  ", 2, 1, gen_lst->f);
		}
	}
}

PUB_FUNC void tcc_listing_code(int codelen, int symoffset, const char *fmt, ...)
{
    va_list ap;
	int idx;
	Section *section;
	...
	while (gen_lst->lastind < ind - codelen)
	{
		int len;
		int end;

		end = (gen_lst->lastind + 16) & ~15;
		if (end > ind - codelen)
			end = ind - codelen;
		
		len = end - gen_lst->lastind;
		if (len > 16)
			len = 16;

		listing_dump(section, gen_lst->lastind, len, -1, 16, 1);
		tcc_listing_print("\n");
		gen_lst->lastind += len;
	}

	if (codelen && gen_lst->lastind == ind - codelen)
	{
		listing_dump(section, gen_lst->lastind, codelen, 
			symoffset > 0 ? gen_lst->lastind + symoffset : -1, 8, 0);
	    va_start(ap, fmt);
		tcc_listing_vprint(fmt, ap);
	    va_end(ap);
		tcc_listing_print("\n");
		gen_lst->lastind += codelen;
	}
}

PUB_FUNC void tcc_listing_dump_section(Section *section)
{
	int offset;
	...
	tcc_listing_print("\n");
	for (offset = 0; offset < section->data_offset; offset += 16)
	{
		int len = min(16, section->data_offset - offset);
		listing_dump(section, offset, len, -1, 16, 1);
		tcc_listing_print("\n");
	}
}
