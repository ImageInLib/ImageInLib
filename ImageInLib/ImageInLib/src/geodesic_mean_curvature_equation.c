	...
	for (k = 0, k_ext = 1; k < height; k++, k_ext++)
	{
		for (i = 0, i_ext = 1; i < length; i++, i_ext++)
		{
			for (j = 0, j_ext = 1; j < width; j++, j_ext++)
			{
				...
				coefPtr_e[k][x] = voxel_coef * presmoot_e_coefPtr[k][x] * (1 / orig_e_coefPtr[k][x]);//east coefficient
				coefPtr_w[k][x] = voxel_coef * presmoot_w_coefPtr[k][x] * (1 / orig_w_coefPtr[k][x]);//west coefficient
				coefPtr_n[k][x] = voxel_coef * presmoot_n_coefPtr[k][x] * (1 / orig_n_coefPtr[k][x]);//north coefficient
				coefPtr_s[k][x] = voxel_coef * presmoot_s_coefPtr[k][x] * (1 / orig_s_coefPtr[k][x]);//south coefficient
				coefPtr_t[k][x] = voxel_coef * presmoot_t_coefPtr[k][x] * (1 / orig_t_coefPtr[k][x]);//top coefficient
				coefPtr_b[k][x] = voxel_coef * presmoot_b_coefPtr[k][x] * (1 / orig_b_coefPtr[k][x]);//bottom coefficient
			}
		}
	}
