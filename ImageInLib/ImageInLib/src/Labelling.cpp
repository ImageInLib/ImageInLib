...
bool new_labelling2D(dataType* imageDataPtr, dataType* segmentedImage, const size_t xDim, const size_t yDim, int object) {
	...
	vector<point2dLabelling> inProcess, temp;

	bool* statusArray = new bool[xDim * yDim];

	size_t currentIndx = 0;

	point2dLabelling currentPoint = {0, 0, false};

	//Initialization
	for (size_t i = 0; i < xDim; i++) {
		for (size_t j = 0; j < yDim; j++) {
			currentIndx = x_new(i, j, xDim);
			statusArray[currentIndx] = false;
		}
	}
	...
	for (i = 0; i < xDim; i++) {
		for (j = 0; j < yDim; j++) {
			//currentPoint = {}
			if (statusArray[x_new(i, j, xDim)] == false) {
				if (imageDataPtr[x_new(i, j, xDim)] == object) {
					//top neighbor
					...
					//after checking all neighbors of current element
					//we give its label, and update its status
					segmentedImage[x_new(i, j, xDim)] = label;
					statusArray[x_new(i, j, xDim)] = true;
					//Now we check neighbors of its neighbors saved in the stacks
					//we start by the last added element in stacks
					//If there is no neighbor iStack.size() = jStack.size() = 0, and the while loop will no be ran
					while (iStack.size() > 0 && jStack.size() > 0) { //One is enought because they have same size
